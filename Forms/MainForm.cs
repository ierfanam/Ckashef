...
public sealed class MainForm : Form
{
    ...
    private void BuildChrome()
    {
        ...
        string[] items = { "داشبورد", "عملیات", "شناسایی‌ها", "اسکن مجاز", "گزارش‌ها", "پرسنل", "تنظیمات" }; var y = 8;
    ...
    {
        ClearContent(); var s=_db.GetDashboardStats(); _content.Controls.Add(UiTheme.SectionTitle("داشبورد عملیاتی"));
        ...
        var grid=new DataGridView{Dock=DockStyle.Fill};UiTheme.StyleDataGrid(grid);grid.Columns.Add("Province","استان");grid.Columns.Add("Count","تعداد");grid.Columns.Add("Power","مصرف (W)");foreach(var x in _db.GetRegionalStats())grid.Rows.Add(x.Province,x.Count,x.Power.ToString("F0"));_content.Controls.Add(grid);_content.Controls.Add(cards);SetStatus("داشبورد عملیاتی به‌روز شد");
    }
    ...
    private async Task StartScan(){if(_scanCts!=null)return;var op=_scanOpCode?.Text.Trim();if(string.IsNullOrWhiteSpace(op)){MessageBox.Show("کد عملیات مجاز الزامی است.");return;}var ips=(_ipList?.Text??"").Split(new[]{'\r','\n'},StringSplitOptions.RemoveEmptyEntries|StringSplitOptions.TrimEntries);if(ips.Length==0&&!(_simMode?.Checked??false)){MessageBox.Show("حداقل یک IP مجاز وارد کنید.");return;}try{_scanCts=new CancellationTokenSource();SetStatus("اسکن مجاز در حال اجراست…");await _tracker.RunAuthorizedScanAsync(op,_scanRegion?.Text.Trim()??"",ips,TrackingService.MinerPorts,_simMode?.Checked??false,AppSession.CurrentUser?.FullName??"سیستم",_scanCts.Token);SetStatus("اسکن با موفقیت پایان یافت");}catch(OperationCanceledException){SetStatus("اسکن متوقف شد");}catch(Exception ex){MessageBox.Show("خطای اسکن: "+ex.Message);}finally{_scanCts?.Dispose();_scanCts=null;}}

    private void ShowReports(){ClearContent();_content.Controls.Add(UiTheme.SectionTitle("گزارش‌ها"));var bar=new FlowLayoutPanel{Dock=DockStyle.Top,Height=54,FlowDirection=FlowDirection.RightToLeft};var all=new Button{Text="گزارش همه عملیات",Width=145};var csv=new Button{Text="خروجی CSV",Width=120};var current=new Button{Text="گزارش عملیات انتخابی",Width=155};UiTheme.StylePrimaryButton(all);UiTheme.StyleSecondaryButton(csv);UiTheme.StyleSecondaryButton(current);all.Click+=(_,_)=>GenerateReport(null);csv.Click+=(_,_)=>ExportReport(null);current.Click+=(_,_)=>ChooseOperationForReport();bar.Controls.AddRange(new Control[]{all,current,csv});_content.Controls.Add(bar);var info=UiTheme.MakeCard("گزارش رسمی عملیات",180);info.Controls.Add(new Label{Text="گزارش‌ها شامل داده‌های ثبت‌شده، وضعیت اقدام، مشخصات فنی و فیلدهای مشترک موجود در پایگاه داده است. اطلاعات هویتی تنها در صورت ورود از منبع رسمی نمایش داده می‌شود.",Dock=DockStyle.Fill,ForeColor=UiTheme.TextMuted,TextAlign=ContentAlignment.MiddleRight,Padding=new Padding(12)});_content.Controls.Add(info);}
    private void GenerateReport(string? op){try{var path=_reports.GenerateTextReport(op,AppSession.CurrentUser?.FullName??"سیستم");MessageBox.Show($"گزارش ایجاد شد:\n{path}","Ckashef");}catch(Exception ex){MessageBox.Show("خطای تولید گزارش: "+ex.Message);}}
    private void ExportReport(string? op){try{var path=_reports.ExportCsv(op,AppSession.CurrentUser?.FullName??"سیستم");MessageBox.Show($"فایل CSV ایجاد شد:\n{path}","Ckashef");}catch(Exception ex){MessageBox.Show("خطای خروجی: "+ex.Message);}}
    private void ChooseOperationForReport(){using var d=new Form{Text="انتخاب عملیات",ClientSize=new Size(360,150),StartPosition=FormStartPosition.CenterParent,RightToLeft=RightToLeft.Yes,RightToLeftLayout=true};var combo=new ComboBox{Dock=DockStyle.Top,Height=35,DropDownStyle=ComboBoxStyle.DropDownList};combo.Items.AddRange(_db.GetOperations().Select(x=>x.OperationCode).Cast<object>().ToArray());if(combo.Items.Count>0)combo.SelectedIndex=0;var ok=new Button{Text="تولید گزارش",Dock=DockStyle.Bottom,Height=40,DialogResult=DialogResult.OK};UiTheme.StylePrimaryButton(ok);d.Controls.Add(ok);d.Controls.Add(combo);if(d.ShowDialog(this)==DialogResult.OK&&combo.SelectedItem!=null)GenerateReport(combo.SelectedItem.ToString());}
    ...
    private void OnScanProgress(object? sender, ScanProgressEventArgs e){if(IsDisposed)return;BeginInvoke(new Action(()=>{_progress.Value=Math.Clamp(e.Percent,0,100);_status.Text=$"● {e.Message}";}));}
