...
/// <summary>
/// Production startup guard. Removes development bootstrap artifacts and makes
/// provenance/audit fields mandatory infrastructure for operational records.
/// </summary>
public static class ProductionHardening
{
    public static void Apply(DatabaseService db)
    {
        ...
        // Never ship the built-in demo operation or the known development admin.
        Execute(conn, tx, "DELETE FROM DetectedOperations WHERE OperationCode = 'OP-DEMO-001'");
        Execute(conn, tx, "DELETE FROM GovernmentOperations WHERE OperationCode = 'OP-DEMO-001'");
        Execute(conn, tx, "DELETE FROM AuthorizedPersonnel WHERE Badge = 'ADMIN-001'");
