...
/// <summary>
/// Authorized network observation. It never derives subscriber identity from an IP address.
/// Identity/address/phone fields must come from an explicitly authorized official data source.
/// </summary>
public sealed class TrackingService
{
    ...
    public Task<List<DetectedDevice>> ScanAsync(string operationCode, IReadOnlyList<string> targetIps, CancellationToken ct = default) =>
        RunAuthorizedScanAsync(operationCode, "", targetIps, MinerPorts, false, AppSession.CurrentUser?.FullName ?? "سیستم", ct);
    ...
    public async Task<List<DetectedDevice>> RunAuthorizedScanAsync(string operationCode, string region, IReadOnlyList<string> targetIps, IReadOnlyList<int> ports, bool simulationMode, string authorizedBy, CancellationToken ct = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(authorizedBy) || !AppSession.IsAuthenticated) throw new UnauthorizedAccessException("اسکن فقط برای کاربر احراز‌شده مجاز است.");
        ...
        if (simulationMode)
        {
            ...
            for (var i=0;i<simCount;i++) { ct.ThrowIfCancellationRequested(); await Task.Delay(180,ct); var m=KnownMiners[rnd.Next(KnownMiners.Length)]; var d=BuildSimulation(operationCode,region,$"SIM-{i+1:000}",portsToUse[rnd.Next(portsToUse.Length)],m,rnd); found.Add(d); _db.SaveDetection(d); Report(i+1,simCount,found.Count,$"SIMULATION: {d.IPAddress}"); }
        }
        else
        {
            var total=targetIps.Count*portsToUse.Length; var scanned=0;
            foreach(var rawIp in targetIps)
            {
                ct.ThrowIfCancellationRequested(); var ip=rawIp.Trim();
                ...
                foreach(var port in portsToUse)
                {
                    ...
                    {
                        // A listening port is evidence of network reachability only. It does not prove device model or power draw.
                        var d=BuildObservation(operationCode,region,ip,port);
                        found.Add(d); _db.SaveDetection(d);
                    }
                    if(scanned%3==0||scanned==total) Report(scanned,total,found.Count,$"بررسی {ip}:{port}");
                }
            }
        }
        _db.SaveScanHistory(operationCode,simulationMode?"Simulation":"AuthorizedScan",start,DateTime.Now,simulationMode?found.Count:targetIps.Count*portsToUse.Length,found.Count,$"مجازکننده: {authorizedBy} | منطقه: {region}");
        Report(100,100,found.Count,simulationMode?"شبیه‌سازی تکمیل شد":"اسکن تکمیل شد"); return found;
    }
    ...
    {
        var localIps=GetLocalIpv4Addresses(); return await RunAuthorizedScanAsync(operationCode,region,localIps,new[]{3333,4028,8333,9332,8080},false,AppSession.CurrentUser?.FullName??"سیستم",ct);
    }

    private static DetectedDevice BuildObservation(string op,string region,string ip,int port)=>new()
    {
        OperationID=Guid.NewGuid().ToString("N"),OperationCode=op,IPAddress=ip,Port=port,Province=region??"",DeviceModel="تأیید نشده",HashRate="تأیید نشده",EstimatedConsumption=0,
        DetectionTime=DateTime.Now,ActionStatus="منتظر‌دستورالعمل",Confidence=1.0,EvidenceType="NetworkObservation",DataSource="NetworkProbe",MatchStatus="Unresolved",
        ActionNotes="وجود سرویس شبکه مشاهده شد؛ مدل دستگاه، توان مصرفی و هویت مشترک از این مشاهده قابل استنتاج نیست."
    };

    private static DetectedDevice BuildSimulation(string op,string region,string ip,int port,MinerProfile m,Random rnd)=>new()
    {
        OperationID=Guid.NewGuid().ToString("N"),OperationCode=op,IPAddress=ip,Port=port,Province=region??"",DeviceModel=m.Name,HashRate=m.HashRate,EstimatedConsumption=m.PowerWatts,
        DetectionTime=DateTime.Now,ActionStatus="منتظر‌دستورالعمل",Confidence=Math.Round(.72+rnd.NextDouble()*.25,2),EvidenceType="Simulation",DataSource="Synthetic",MatchStatus="Simulation",
        ActionNotes="SIMULATION — داده مصنوعی است و نباید به‌عنوان واقعیت عملیاتی یا هویت مشترک استفاده شود."
    };
